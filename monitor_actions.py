#!/usr/bin/env python3
"""
CGV + 메가박스 무대인사/GV/시네마톡 모니터링 (GitHub Actions용)
"""
...
def main():
    ...
    if cgv_greetings is None:
        cgv_greetings = []
    print(f"CGV: {len(cgv_greetings)}개 발견")
    ...
    if megabox_greetings is None:
        megabox_greetings = []
    print(f"메가박스: {len(megabox_greetings)}개 발견")

    # 합치기
    greetings = cgv_greetings + megabox_greetings
    print(f"\n총 {len(greetings)}개 이벤트 발견 (CGV: {len(cgv_greetings)}, 메가박스: {len(megabox_greetings)})")
    ...
    if not saved_data.get("greetings"):
        ...
        if greetings and DISCORD_WEBHOOK_URL:
            requests.post(DISCORD_WEBHOOK_URL, json={
                "content": f"✅ CGV + 메가박스 무대인사 모니터링 시작!\n{len(greetings)}개 이벤트 추적 중 (CGV: {len(cgv_greetings)}, 메가박스: {len(megabox_greetings)})"
            }, timeout=10)
        return
